...
const MyComponent: React.FC<MyComponentProps> = ({ /* Destructure props here */ }) => {
  ...
  return (
    <div className=" flex flex-col bg-[#04001f] h-screen overflow-y-hidden">
      ...
      <FontAwesomeIcon icon={faArrowCircleLeft} className="cursor-pointer text-[#b3afaf] w-[50px] h-[30px]"/>
      <FontAwesomeIcon icon={faArrowCircleRight} className="cursor-pointer text-[#b3afaf] w-[50px] h-[30px]"/>
      </div>
      </div>
      <div className=" absolute bottom-[14%] text-[1px] left-[1%] w-[3%]">
       <a href=""><FontAwesomeIcon icon={faLinkedin} className=" text-[#b3afaf] w-[23px] h-[30px]"/></a> 
